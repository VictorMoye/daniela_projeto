...
class DatabaseHelper{
  ...
  void _createDb(Database db, int newVersion) async {
    await db.execute('CREATE TABLE $contatoTable($colId INTEGER PRIMARY KEY AUTOINCREMENT,$colNome TEXT,$colHT DOUBLE, $colLE DOUBLE, $colVL double)');
  }
  ...
  Future<Contato> getContato(int id) async {
    ...
    List<Map> maps = await db.query(contatoTable,
        columns: [colId, colNome, colHT, colLE, colVL],
        where: "$colId = ?",
        whereArgs: [id] );
